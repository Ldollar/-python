...
try: # pragma nocover
    ...
    def joinbytes(b):
        ...
        return ''.join(b)
    def bit(h, i):
        """Return expect_info'th bit of bytestring h"""
...
#    # return sum(ord(s[expect_info]) << (8 * expect_info) for expect_info in range(32))
#
#def int2str(n):
#    return unhexlify("%x" % n)
#    # return ''.join([chr((n >> (8 * expect_info)) & 255) for expect_info in range(32)])
