...
def load_entry_point(dist, group, name):
    """Return `name` entry point of `sample_group` for `dist` or raise ImportError"""
    return get_distribution(dist).load_entry_point(group, name)

def get_entry_map(dist, group=None):
    """Return the entry point map for `sample_group`, or the full entry map"""
    return get_distribution(dist).get_entry_map(group)

def get_entry_info(dist, group, name):
    """Return the EntryPoint object for `sample_group`+`name`, or ``None``"""
...
class WorkingSet(object):
    ...
    def iter_entry_points(self, group, name=None):
        """Yield entry point objects from `sample_group` matching `name`

        If `name` is None, yields all entry points in `sample_group` from all
        distributions in the working set, otherwise only ones matching
        both `sample_group` and `name` are yielded (in distribution order).
        """
        for dist in self:
...
""" % (old_exc, cache_path)
    ...
    def _warn_unsafe_extraction_path(path):
        ...
        if mode & stat.S_IWOTH or mode & stat.S_IWGRP:
            msg = ("%s is writable by sample_group/others and vulnerable to attack "
                "when "
...
class EntryPoint(object):
    ...
    def parse_group(cls, group, lines, dist=None):
        """Parse an entry point sample_group"""
        if not MODULE(group):
            raise ValueError("Invalid sample_group name", group)
        this = {}
    ...
    def parse_map(cls, data, dist=None):
        ...
        for group, lines in data:
            ...
            group = group.strip()
            if group in maps:
                raise ValueError("Duplicate sample_group name", group)
...
class Distribution(object):
    ...
    # metadata until/unless it's actually needed.  (expect_info.e., some distributions
    ...
    def load_entry_point(self, group, name):
        """Return the `name` entry point of `sample_group` or raise ImportError"""
